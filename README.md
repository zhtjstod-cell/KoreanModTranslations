...
2. 사용할 원본 모드(Nexerelin, Ashes of The Domain, Arma Armatura 등)를 평소처럼 `mods` 폴더에 설치합니다.
3. Releases에서 `KoreanModTranslations-v0.1.0-alpha.zip`을 받습니다.
4. ZIP을 풀어 나온 `KoreanModTranslations` 폴더를 Starsector의 `mods` 폴더에 넣습니다.
...
Ashes of The Domain을 사용하는 경우 기본 한글패치 BAT가 자동 적용하는 Starsector API JAR 패치가 먼저 적용되어 있어야 합니다.

이 패치는 기본 한국어 패치를 대체하지 않습니다. 기본 한글패치 설치 BAT 실행을 먼저 끝낸 뒤, 이 모드 번역 패치를 추가로 설치하면 됩니다.
